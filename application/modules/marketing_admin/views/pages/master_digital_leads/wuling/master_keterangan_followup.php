...
<script>
    ...
    function edit_data(dataId) {
        ...
        $.ajax({
            method: 'POST',
            url: '<?= base_url() ?>digital_leads/wuling_master_keterangan_followup/ambil',
            data: 'id=' + id,
            dataType: "json",
    ...
    function hapus_data(dataId) {
        ...
        if (confirm("Anda yakin ingin menghapus data ini??")) {
            $.ajax({
                type: 'POST',
                url: "<?php echo site_url(); ?>digital_leads/wuling_master_keterangan_followup/hapus",
                data: 'id=' + id,
                cache: false,
                success: function(data) {
    ...
    $('#simpan').click(function(e) {
        ...
        var nama_keterangan_fu = $('#nama_keterangan_fu_input').val()
        if (cek_input(nama_keterangan_fu, 'keterangan_customer', 'nama_keterangan_fu')) {
            if (confirm("Anda yakin ingin menyimpan data ini??")) {
                $.ajax({
                    type: 'POST',
                    url: "<?php echo site_url(); ?>digital_leads/wuling_master_keterangan_followup/simpan",
                    data: string,
                    cache: false,
    ...
    $('#tambah').click(function() {
        $('#id_keterangan_fu_input').val('')
        $('#nama_keterangan_fu_input').val('')
    })
    ...
    $(document).ready(function() {
        $('#tabel-data').DataTable({
            ...
            order: [],
            ajax: {
                url: "<?= base_url() ?>digital_leads/wuling_master_keterangan_followup/semua_data",
                type: 'POST',
            },
            columns: [{
                    data: "id_keterangan_fu",
                    title: "No.",
                ...
                },
                {
                    ...
                    <button type="button" onclick="edit_data(` + data.id_keterangan_fu + `);" class="btn btn-info" data-toggle="modal" data-target="#modal"><i class="icon-ios-compose-outline"></i> Edit</button>
                    <button type="button" onclick="hapus_data(` + data.id_keterangan_fu + `);" class="btn btn-danger"> Hapus</button>
                    </div></div>`;
                    },
                }
            ]

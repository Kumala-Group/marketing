...
<script>
    ...
    function edit_data(dataId) {
        ...
        $.ajax({
            method: 'POST',
            url: '<?= base_url() ?>digital_leads/wuling_master_status_customer/ambil',
            data: 'id=' + id,
            dataType: "json",
    ...
    function hapus_data(dataId) {
        ...
        if (confirm("Anda yakin ingin menghapus data ini??")) {
            $.ajax({
                type: 'POST',
                url: "<?php echo site_url(); ?>digital_leads/wuling_master_status_customer/hapus",
                data: 'id=' + id,
                cache: false,
                success: function(data) {
    ...
    $('#simpan').click(function(e) {
        ...
        var nama_status_customer = $('#nama_status_customer_input').val()
        if (cek_input(nama_status_customer, 'status_customer', 'nama_status_customer')) {
            if (confirm("Anda yakin ingin menyimpan data ini??")) {
                $.ajax({
                    type: 'POST',
                    url: "<?php echo site_url(); ?>digital_leads/wuling_master_status_customer/simpan",
                    data: string,
                    cache: false,
    ...
    $('#tambah').click(function() {
        $('#id_status_customer_input').val('')
        $('#nama_status_customer_input').val('')
    })
    ...
    $(document).ready(function() {
        $('#tabel-data').DataTable({
            ...
            order: [],
            ajax: {
                url: "<?= base_url() ?>digital_leads/wuling_master_status_customer/semua_data",
                type: 'POST',
            },
            columns: [{
                    data: "id_status_customer",
                    title: "No.",
                ...
                },
                {
                    ...
                    render: function(data) {
                        const id_status = ['1', '3', '6', '7']
                        let btn_edit = ''
                        let btn_del = ''
                        if (!id_status.includes(data.id_status_customer)) {
                            btn_edit = `<button type="button" onclick="edit_data(${data.id_status_customer});" class="btn btn-info" data-toggle="modal" data-target="#modal"><i class="icon-ios-compose-outline"></i> Edit</button>`
                            btn_del = `<button type="button" onclick="hapus_data(${data.id_status_customer});" class="btn btn-danger"> Hapus</button>`
                        }

        ...
        <div class="col-md-12">
            <div class="card" style="border: 0;box-shadow: none;">
                <div class="card-header">
                    <h5 class="card-title mb-0">Data Aktivitas Prospek</h5>
                ...
                <div class="card-body collapse in">
                    <div class="card-block p-0">
                        <div class="row">
                            <div class="col-md-12">
                                <form class="form_line form-inline">
                                    ...
                                    <div class="form-group mb-1">
                                        <button id="button_sales" name="button_sales" class="btn btn-success">
                                            <i class="icon-ios-eye"></i> Lihat Detail Per Sales
                                        </button>
                                    </div>
                                </form>
                            </div>
                        ...
                        <div class="row my-2">
                            <div class="col-md-6">
                                <div id="linechart_prospek"></div>
                            </div>
                            <div class="col-md-6">
                                <div id="chart_prospek"></div>
                            </div>
                        </div>
                        <div class="row my-2">
                            <div class="col-md-12">
        ...
        if (form.valid() && perusahaan.length != 0) {
            ...
            $.post(location, {
                ...
                function(r) {
                    ...
                    $('#view').html(r.view);
                    $('#view_detail').html('');
                    google.charts.setOnLoadCallback(linechart_prospek(bulan, tahun, perusahaan));
                    google.charts.setOnLoadCallback(chart_prospek(bulan, tahun, perusahaan));
                }, "json"
            );
        }
    });
    $('#button_sales').click(function(e) {
        e.preventDefault();
        form = $('.form_line');
        var perusahaan = $('#perusahaan').val();
        var bulan = $('#bulan').val();
        ...
        if (form.valid() && perusahaan.length != 0) {
            ...
            $.post(location, {
                ...
                function(r) {
                    $('.card').addClass('card-fullscreen');
                    var buttonState = localStorage.getItem('saveButtonState');
                    $('#button_sales').prop('disabled', false);
                    $('#button_sales').html(buttonState);

                    $('#view_detail').html(r.view_detail);
                }, "json"
            );
        }
    });

    function linechart_prospek(bln, thn, perusahaan) {
        var jsonData = $.ajax({
            type: "POST",
            url: location,
            dataType: "json",
            data: {
                load_linechart_prospek: true,
                perusahaan: perusahaan,
                bln: bln,
                thn: thn
            },
        ...
        var chart = new google.charts.Line(document.getElementById('linechart_prospek'));
        chart.draw(data, google.charts.Line.convertOptions({
            'title': 'Trafik Aktivitas Prospek',
            width: 800,
            height: 400
        }));
    }

    function chart_prospek(bln, thn, perusahaan) {
        var jsonData = $.ajax({
            type: "POST",
            url: location,
            dataType: "json",
            data: {
                load_chart_prospek: true,
                perusahaan: perusahaan,
                bln: bln,
                thn: thn
            },
        ...
        // Instantiate and draw our chart, passing in some options. 
        var chart = new google.visualization.PieChart(document.getElementById('chart_prospek'));
        chart.draw(data, {
            'title': 'Chart Aktivitas Prospek',
            width: 500,
            height: 400
        });
    }

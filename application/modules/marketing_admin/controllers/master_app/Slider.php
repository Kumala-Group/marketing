...
class Slider extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($this->m_marketing->auth_login('admin_it,adm_mrktng', $index)) {
            ...
            if ($post) {
                ...
                $d['beranda']     = q_data("*", 'kumalagroup.sliders', "kategori like '%beranda%'", "updated_at")->result();
                $d['hino']        = q_data("*", 'kumalagroup.sliders', "kategori like '%hino%'", "updated_at")->result();
                $d['honda']       = q_data("*", 'kumalagroup.sliders', "kategori like '%honda%'", "updated_at")->result();
                $d['wuling']      = q_data("*", 'kumalagroup.sliders', "kategori like '%wuling%'", "updated_at")->result();
                $d['mazda']       = q_data("*", 'kumalagroup.sliders', "kategori like '%mazda%'", "updated_at")->result();
                $d['carimobilku'] = q_data("*", 'kumalagroup.sliders', "kategori like '%carimobilku%'", "updated_at")->result();
    ...
    function simpan($post)
    {
        ...
        $q_brand = q_data("*", 'kumalagroup.sliders', $where);
        if ($q_brand->num_rows() == 0) {
            ...
            if (!empty($_FILES['gambar'])) {
                $data['gambar'] = $nama_gambar;
                $data_post['name'] = q_data("*", 'kumalagroup.sliders', $where)->row()->gambar;
    ...
    {
        $where['id'] = $post['id'];
        $data = q_data("*", 'kumalagroup.sliders', $where)->row();

...
class Lokasi_event extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($this->m_marketing->auth_login('admin_it,adm_mrktng,adm_marksup', $index)) {
            ...
            if ($post) {
                ...
                $d['content'] = "pages/marketing_support/wuling/lokasi_event";
                $d['index'] = $index;
                $d['area'] = q_data("*", 'db_wuling.event_area', ['id_perusahaan' => $this->session->userdata('id_perusahaan')])->result();
    ...
    function simpan($post)
    {
        ...
        $q_level = q_data("*", 'db_wuling.event_lokasi', $where);
        if ($q_level->num_rows() == 0) {
            $this->db_wuling->insert("event_lokasi", $data);
            $status = 1;
        } elseif ($q_level->num_rows() > 0 && !empty($where)) {
            $this->db_wuling->update("event_lokasi", $data, $where);
            $status = 2;
        }
        echo $status;
    }
    ...
    {
        $where['id_event_lokasi'] = $post['id'];
        $data = q_data("*", 'db_wuling.event_lokasi', $where)->row();
    ...
    {
        $where['id_event_lokasi'] = $post['id'];
        echo $this->db_wuling->delete('event_lokasi', $where) ? 1 : 0;
    }
}

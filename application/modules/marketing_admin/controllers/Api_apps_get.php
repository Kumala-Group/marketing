...
class Api_apps_get extends CI_Controller
{
    ...
    public function m_berita()
    {
        if ($this->m_marketing->auth_api()) {
            ...
            echo json_encode(q_data("*", 'kumk6797_kumalagroup.beritas', ['type' => $type], "created_at")->result());
        }
    }
    public function m_brand()
    {
        if ($this->m_marketing->auth_api()) echo json_encode(q_data("*", 'kumk6797_kumalagroup.brands', [])->result());
    }
    public function m_dealer()
    {
        if ($this->m_marketing->auth_api()) {
            $d = [];
            if ($this->uri->segment(5)) $d = q_data("*", 'kumk6797_kumalagroup.dealers', ['brand' => $this->uri->segment(4), 'area' => $this->uri->segment(5)])->result();
            elseif (is_numeric($this->uri->segment(4))) {
                $area = q_data("*", 'kumk6797_kumalagroup.dealers', ['brand' => $this->uri->segment(4)], [], "area")->result();
                foreach ($area as $v) {
                    $arr['id'] = $v->id;
                    $arr['brand'] = $v->brand;
                    $arr['area'] = $v->area;
                ...
                if ($uri[0] == "detail") $d = q_data("*", 'kumk6797_kumalagroup.dealers', ['id' => end($uri)])->row();
                else $d = q_data("*", 'kumk6797_kumalagroup.dealers', ['brand' => end($uri)])->result();
            } else $d = q_data("*", 'kumk6797_kumalagroup.dealers', [])->result();
            echo json_encode($d);
        }
    }
    ...
    {
        if ($this->m_marketing->auth_api()) {
            if ($this->uri->segment(4)) $d = q_data("*", 'kumk6797_kumalagroup.events', ['id' => $this->uri->segment(4)])->row();
            else $d = q_data("*", 'kumk6797_kumalagroup.events', [], "updated_at")->result();
            echo json_encode($d);
        }
    }
    public function m_voucher()
    {
        if ($this->m_marketing->auth_api()) echo json_encode(q_data("*", 'kumk6797_kumalagroup.vouchers', [], "updated_at")->result());
    }
    public function m_produk()
    {
        if ($this->m_marketing->auth_api()) {
            ...
            if ($this->uri->segment(5)) {
                $warna = q_data("*", 'kumk6797_kumalagroup.units_detail', ['unit' => $this->uri->segment(5), 'detail' => "warna", 'gambar!=' => ""])->result();
                foreach ($warna as $v) {
                    $arr['id'] = $v->id;
                    $arr['nama_detail'] = q_data("*", 'kumk6797_kumalagroup.colors', ['id' => $v->nama_detail])->row()->nama_warna;
                    $arr['deskripsi'] = "0xFF" . $v->deskripsi;
                    $d['warna'][] = $arr;
                }
                $arr = [];
                $spek = q_data("*", 'kumk6797_kumalagroup.units_detail', ['unit' => $this->uri->segment(5), 'detail' => "spek"])->result();
                foreach ($spek as $v) {
                ...
                $arr = [];
                $detail = q_data("*", 'kumk6797_kumalagroup.units_detail', ['unit' => $this->uri->segment(5), 'detail' => "detail"])->result();
                ...
                $data = q_data("*", 'kumk6797_kumalagroup.units', ['brand' => $this->uri->segment(4)], "updated_at")->result();
                foreach ($data as $v) {
                    $arr['id'] = $v->id;
    ...
    {
        if ($this->m_marketing->auth_api()) echo json_encode(q_data("*", 'kumk6797_kumalagroup.units_detail', ['id' => $this->uri->segment(4)])->row());
    }
    public function m_sparepart()
    {
        if ($this->m_marketing->auth_api()) {
            $d = [];
            if ($this->uri->segment(5)) $d = q_data("*", 'kumk6797_kumalagroup.spareparts', ['id' => $this->uri->segment(5)], "updated_at")->row();
            else {
                $data = q_data("*", 'kumk6797_kumalagroup.spareparts', ['brand' => $this->uri->segment(4)], "updated_at")->result();
                foreach ($data as $v) {
                    $arr['id'] = $v->id;
                    $arr['nama'] = $v->nama;
                    $arr['harga'] = $v->harga;
    ...
    {
        if ($this->m_marketing->auth_api()) {
            $data = q_data("*", 'kumk6797_kumalagroup.customer', ['id' => $this->uri->segment(4)])->row();
            if ($data) {
                $reg = q_data("*", 'kumk6797_kumalagroup.reg_customer', ['id' => $data->customer])->row();
                $d['nama'] = $reg->nama;
                $d['email'] = $reg->email;

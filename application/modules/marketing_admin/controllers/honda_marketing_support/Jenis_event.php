...
class Jenis_event extends CI_Controller
{
        ...
        if ($this->m_marketing->auth_login('admin_it,adm_mrktng,adm_marksup', $index)) {
            ...
            if ($post) {
                ...
                elseif (!empty($post['datatable']))
                    echo q_datatable("id_event_jenis,event_jenis", 'db_honda.event_jenis', []);
            } else {
                $d['content'] = "pages/marketing_support/honda/jenis_event";
                $d['index'] = $index;
    ...
    function simpan($post)
    {
        ...
        $q_level = q_data("*", 'db_honda.event_jenis', $where);
        if ($q_level->num_rows() == 0) {
            $this->db_honda->insert("event_jenis", $data);
            $status = 1;
        } elseif ($q_level->num_rows() > 0 && !empty($where)) {
            $this->db_honda->update("event_jenis", $data, $where);
            $status = 2;
        }
        echo $status;
    }
    ...
    {
        $where['id_event_jenis'] = $post['id'];
        $data = q_data("*", 'db_honda.event_jenis', $where)->row();
    ...
    {
        $where['id_event_jenis'] = $post['id'];
        echo $this->db_honda->delete('event_jenis', $where) ? 1 : 0;
    }
}

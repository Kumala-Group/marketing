...
class Sumber_prospek extends CI_Controller
{
        ...
        if ($this->m_marketing->auth_login('admin_it,adm_mrktng,adm_marksup', $index)) {
            ...
            if ($post) {
                ...
                elseif (!empty($post['datatable']))
                    echo q_datatable("id_sumber_prospek,sumber_prospek", 'db_honda.p_sumber_prospek', []);
            } else {
                $d['content'] = "pages/marketing_support/honda/sumber_prospek";
                $d['index'] = $index;
    ...
    function simpan($post)
    {
        ...
        $q_level = q_data("*", 'db_honda.p_sumber_prospek', $where);
        if ($q_level->num_rows() == 0) {
            $this->db_honda->insert("p_sumber_prospek", $data);
            $status = 1;
        } elseif ($q_level->num_rows() > 0 && !empty($where)) {
            $this->db_honda->update("p_sumber_prospek", $data, $where);
            $status = 2;
        }
        echo $status;
    }
    ...
    {
        $where['id_sumber_prospek'] = $post['id'];
        $data = q_data("*", 'db_honda.p_sumber_prospek', $where)->row();
    ...
    {
        $where['id_sumber_prospek'] = $post['id'];
        echo $this->db_honda->delete('p_sumber_prospek', $where) ? 1 : 0;
    }
}

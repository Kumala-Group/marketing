                    ...
                    <div class="card-body collapse in">
                        <div class="card-block">
                            ...
                            <div style="height: 50%;width: 100%;overflow-x: auto;overflow-y: auto;white-space: nowrap;">
                                <table class="table table-sm table-bordered" id="tabel-pengeluaran-operasional">
                                </table>
                            </div>
                            </div>
                        </div>
...
<script type="text/javascript">
    ...
    var datatables = $('#tabel-pengeluaran-operasional').DataTable({
        // scrollY: 450,
        processing: true,
        serverSide: true,
        order: [],
        // responsive: true,
        ajax: {
            url: "<?= base_url(); ?>audit/kumalagroup_bank_pengeluaran/get_pengeluaran_operasional",
            type: 'POST',
        ...
        columns: [{
            ...
            },
            {
                ...
                render: function(data, type, full, meta) {
                    ...
                    return `<input id="` + full.no_pengeluaran + `_status" type="checkbox" ` + status + ` onChange="update_status($(this).is(':checked'), '` + full.no_pengeluaran + `')">`;
                },
            }
        ]
    });
    ...
    function update_status(value, no_pengeluaran) {
        ...
        $.post("<?= base_url(); ?>audit/kumalagroup_bank_pengeluaran/set_status_cek_pengeluaran_operasional_for_audit", {
                'status': status,
            ...
            function(data) {
                if (data == 'updates') {
                    swal("", "Data ini sudah di periksa!", "success").then(function() {
                        $('#' + no_pengeluaran + '_status').attr("checked", false);
                        // if (status == 1) {
                        // } else {
                        //     $('#' + no_pengeluaran + '_status').attr("disabled", false);
                        // }
                    });
                }
            },

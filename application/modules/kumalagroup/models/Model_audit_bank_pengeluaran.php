...
class Model_audit_bank_pengeluaran extends CI_Model
{
    ...
    public function get_pengeluaran_unit()
    {
        ...
        $where = "pu.subgolongan = '110200' and pu.tgl >= '$tgl_awal' and pu.tgl <= '$tgl_akhir' and pu.id_perusahaan = '$cabang' and (pu.no_pengeluaran LIKE '%$no_transaksi%' or pu.no_bukti_bku LIKE '%$no_transaksi%') and pu.kepada LIKE '%$kepada%'";
    ...
    public function get_pengeluaran_after_sales()
    {
        ...
        $where = "pb.subgolongan = '110200' and pb.tgl >= '$tgl_awal' and pb.tgl <= '$tgl_akhir' and pb.id_perusahaan = '$cabang' and (pb.no_pengeluaran LIKE '%$no_transaksi%' or pb.no_bukti_bku LIKE '%$no_transaksi%') and pb.kepada LIKE '%$kepada%'";
    ...
    public function get_pengeluaran_operasional()
    {
        ...
        $no_transaksi = $post['no_transaksi'];
        $kepada = $post['kepada'];
        $select = [null, "po.no_pengeluaran", "po.no_bukti_bku", "po.id_perusahaan", "po.tgl", "a.nama_akun", "po.id_bank", "po.total", "po.keterangan", "po.status_cek_audit"];
        $table  = $nama_tabel . '.pengeluaran_operasional po';
        $join   = [
            $nama_tabel . '.akun a' => 'a.kode_akun = po.subgolongan',
        ];
        $where = "po.subgolongan = '110200' and po.tgl >= '$tgl_awal' and po.tgl <= '$tgl_akhir' and po.id_perusahaan = '$cabang' and (po.no_pengeluaran LIKE '%$no_transaksi%' or po.no_bukti_bku LIKE '%$no_transaksi%') and po.kepada LIKE '%$kepada%'";
    ...
    // UPDATE STATUS CEK AUDIT PENGELUARAN OPERASIONAL
    public function update_status_pengeluaram_operasional_for_audit()
    {
        $nama_tabel = $this->nama_database_brand();
        $status = $this->input->post('status');
        $no_pengeluaran = $this->input->post('no_pengeluaran');
        ...
        $this->$nama_tabel->where('no_pengeluaran', $no_pengeluaran);
        $this->$nama_tabel->update('pengeluaran_operasional', $data_update);
        if ($status == 2) {
            echo "updates";
        }
    }

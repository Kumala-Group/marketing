...
class Henkel_adm_lap_penjualan_sales extends CI_Controller {
	...
	public function index()
	{
		...
		if(!empty($cek)){
			$d['judul']="Laporan Penjualan Per Sales";
			$d['class'] = "laporan";
			$d['data_perusahaan'] = '';
			$d['data_status'] = '';
			$d['content']= 'laporan/penjualan/lap_penjualan_sales';
	...
	public function cari_data()
	{
		...
		if(!empty($cek)){
			...
			$q = $this->model_lap_penjualan->data_laporan_penjualan_sales($tgl_awal, $tgl_akhir);
			$r = $q->num_rows();
			if ($r<1) {
				echo $this->load->view('laporan/view_kosong');
			} else {
				$dt['data'] = $q;
				echo $this->load->view('laporan/penjualan/view_lap_penjualan_sales',$dt);
			}
			...
			if($r>0){
			  ...
			  $pdf->AddPage("P","A4");
				//foreach($data->result() as $t){
					...
					$Q[0]=216;
					$Q[1]=279;
					$pdf->SetTitle('Laporan Penjualan Per Sales');
					...
					$pdf->SetX(6);
					$pdf->Cell(190,4,'Laporan Penjualan Per Sales',0,1,'C');
					$pdf->Ln(2);
					$pdf->SetFont('Times','',10);
					$pdf->SetX(6);
					...
					$pdf->Cell($w[1],$h,'Kode Sales',1,0,'C');
    			...
    			$pdf->SetTextColor(0);
					...
					$total_akhir_f=0;
						foreach($q->result() as $row){
								...
								$pdf->Cell($w[1],$h,$row->kode_sales,'LR',0,'C',$fill);
				...
				//}
				$pdf->Output('Lap_Penjualan_Per_sales - '.date('d-m-Y').'.pdf','I');
			}else{
				redirect('henkel_adm_lap_penjualan_sales');
			}
	...
	public function cetak_excel_penjualan(){
		...
		if(!empty($cek) && $level=='henkel_admin'){
				$tgl_awal  = $this->input->get('tgl_awal');
				$tgl_akhir = $this->input->get('tgl_akhir');
        $ambildata = $this->model_lap_penjualan->data_laporan_penjualan_sales($tgl_awal, $tgl_akhir);
        ...
        if($r>0){
            ...
            $objPHPExcel->getProperties()
                    		...
                    		->setTitle("Export Excel Data Penjualan Per Sales");  //file title
            ...
            $val = array("NO","Kode Sales","Kode Item","Nama Item","Tipe","Jumlah","Harga");
            ...
            foreach ($ambildata->result() as $dt){
							  $harga = $dt->total_item_item_masuk*$dt->harga_tebus_dpp;
								...
								$objset->setCellValue("B".$baris, $dt->kode_sales);
            ...
            $filename = urlencode("Data Laporan Penjualan Per Sales".date("Y-m-d H:i:s").".xls");
            ...
            $objWriter->save('php://output');
					...
					redirect('henkel_adm_lap_penjualan_sales', 'refresh');
        }
			}else{
				redirect('henkel','refresh');
			}

...
class Henkel_adm_setakun_hutangpiutang extends CI_Controller {
	...
	public function index()
	{
		...
		if(!empty($cek)){
			$d['judul']="Setting Akun Penjualan";
			$d['class'] = "akuntansi";
			$d['data_perusahaan'] = '';
			$d['data_status'] = '';
			$d['content']= 'pengaturan/setting_perkiraan/hutang_piutang/view';
	...
	public function cari_data()
	{
		...
		if(!empty($cek)){
			...
			$q = $this->model_lap_penjualan->data_laporan_penjualan_item($tgl_awal, $tgl_akhir);
			$r = $q->num_rows();
			if ($r<1) {
				echo $this->load->view('laporan/view_kosong');
			} else {
				$dt['data'] = $q;
				echo $this->load->view('laporan/penjualan/view_lap_penjualan_item',$dt);
			}
			...
			if($r>0){
			  ...
			  $pdf->AddPage("P","A4");
				//foreach($data->result() as $t){
					...
					$pdf->SetX(6);
					$pdf->Cell(190,4,'Laporan Penjualan Per Item',0,1,'C');
					$pdf->Ln(2);
					$pdf->SetFont('Times','',10);
					$pdf->SetX(6);
				...
				//}
				$pdf->Output('Lap_Penjualan_Per_Item - '.date('d-m-Y').'.pdf','I');
			}else{
				redirect('henkel_adm_lap_penjualan_item');
			}
	...
	public function cetak_excel_penjualan(){
		...
		if(!empty($cek) && $level=='henkel_admin'){
				$tgl_awal  = $this->input->get('tgl_awal');
				$tgl_akhir = $this->input->get('tgl_akhir');
        $ambildata = $this->model_lap_penjualan->data_laporan_penjualan_item($tgl_awal, $tgl_akhir);

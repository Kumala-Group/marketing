...
class Henkel_adm_item_keluar extends CI_Controller {
	...
	public function create_kd()
	{
		...
		if(!empty($cek) && $level=='henkel_admin'){
			...
			if($last_kd > 0){
				$no_akhir = $last_kd+1;
				$kd = "HNKLKEL".sprintf("%03s", $no_akhir);
				//echo json_encode($d);
			}else{
				$kd = 'HNKLKEL001';
				//echo json_encode($d);
	...
	public function cari()
	{
		...
		if(!empty($cek) && $level=='henkel_admin'){
			...
			if($this->model_item_keluar->ada($id)) {
				...
				$d['no_po'] 	= $dt->no_po;
				$data_po = $this->db_kpp->from('pesanan_pembelian')->where('no_po',$dt->no_po)->get();
	...
	public function hapus()
	{
		...
		if(!empty($cek) && $level=='henkel_admin'){
			...
			redirect('henkel_adm_item_keluar','refresh');
		}
		else
		{
			redirect('henkel','refresh');

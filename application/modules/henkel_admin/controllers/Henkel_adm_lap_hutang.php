...
class Henkel_adm_lap_hutang extends CI_Controller {
	...
	public function index()
	{
		...
		if(!empty($cek)){
			$d['judul']="Laporan Hutang Beredar";
			$d['class'] = "laporan";
			$d['data_perusahaan'] = '';
			$d['data_status'] = '';
			$d['content']= 'laporan/hutang/lap_hutang_beredar';
	...
	public function cari_data()
	{
		...
		if(!empty($cek)){
			...
			$q = $this->model_lap_hutang->data_laporan_hutang($tgl_awal, $tgl_akhir);
			$r = $q->num_rows();
			if ($r<1) {
				echo $this->load->view('laporan/view_kosong');
			} else {
				$dt['data'] = $q;
				echo $this->load->view('laporan/hutang/view_lap_hutang_beredar',$dt);
			}
			...
			if($r>0){
			  ...
			  $pdf->AddPage("P","A4");
				//foreach($data->result() as $t){
					...
					$pdf->SetX(6);
					$pdf->Cell(190,4,'Laporan Hutang',0,1,'C');
					$pdf->Ln(2);
					$pdf->SetFont('Times','',10);
					$pdf->SetX(6);
    			...
    			$pdf->SetTextColor(0);
					...
					foreach($q->result() as $row){
						...
						$pdf->Cell($w[3],$h,$row->jumlah_item,'LR',0,'C',$fill);
				...
				//}
				$pdf->Output('Lap_Hutang - '.date('d-m-Y').'.pdf','I');
			}else{
				redirect('henkel_adm_lap_hutang');
			}
	...
	public function cetak_excel_hutang(){
		...
		if(!empty($cek) && $level=='henkel_admin'){
				$tgl_awal  = $this->input->get('tgl_awal');
				$tgl_akhir = $this->input->get('tgl_akhir');
        $ambildata = $this->model_lap_hutang->data_laporan_hutang($tgl_awal, $tgl_akhir);
        ...
        if($r>0){
            ...
            $objPHPExcel->getProperties()
                    		...
                    		->setTitle("Export Excel Data Hutang Beredar");  //file title

            $objset = $objPHPExcel->setActiveSheetIndex(0); //inisiasi set object
            $objget = $objPHPExcel->getActiveSheet();  //inisiasi get object

            $objget->setTitle('Laporan Hutang Beredar');
            ...
            $filename = urlencode("Data Laporan Hutang Beredar".date("Y-m-d H:i:s").".xls");
            ...
            $objWriter->save('php://output');
					...
					redirect('henkel_adm_lap_hutang', 'refresh');
        }
			}else{
				redirect('henkel','refresh');
			}

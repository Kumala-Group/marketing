...
class Henkel_adm_lap_umur_piutang extends CI_Controller {
	...
	public function index()
	{
		...
		if(!empty($cek)){
			$d['judul']="Laporan Umur Piutang";
			$d['class'] = "laporan";
			$d['data_perusahaan'] = '';
			$d['data_status'] = '';
			$d['content']= 'laporan/piutang/lap_umur_piutang';
	...
	public function cari_data()
	{
		...
		if(!empty($cek)){
			...
			$q = $this->model_lap_piutang->data_laporan_umur_piutang($tgl_awal, $tgl_akhir);
			$r = $q->num_rows();
			if ($r<1) {
				echo $this->load->view('laporan/view_kosong');
			} else {
				$dt['data'] = $q;
				echo $this->load->view('laporan/piutang/view_lap_umur_piutang',$dt);
			}
			...
			if($r>0){
			  ...
			  $pdf->AddPage("L","A4");
				//foreach($data->result() as $t){
					...
					$Q[0]=216;
					$Q[1]=279;
					$pdf->SetTitle('Laporan Umur Piutang');
					...
					$pdf->SetX(50);
					$pdf->Cell(190,4,'Laporan Umur Piutang',0,1,'C');
					$pdf->Ln(2);
					...
					$w = array(10,22,22,25,21,15,28,28,28,28,28,28);

					//Header
					$pdf->SetFont('Times','B',9);
					$pdf->Cell($w[0],$h,'No',1,0,'C');
					$pdf->Cell($w[1],$h,'Tgl Transaksi',1,0,'C');
	          ...
	          $hitung_umur = $interval->format('%a');
						...
						$pdf->Cell($w[5],$h,$umur_piutang_total,'LR',0,'C',$fill);
						if ($hitung_umur<=15) {
							$_15+=$total_akhir_o;
						...
						if ($hitung_umur>15 && $hitung_umur<=30) {
							$_30+=$total_akhir_o;
							$pdf->Cell($w[7],$h,'Rp. '.separator_harga2($row->sisa_o),'LR',0,'C',$fill);
            } else {
							$pdf->Cell($w[7],$h,'Rp. 0','LR',0,'C',$fill);
						}
						if ($hitung_umur>31 && $hitung_umur<=45) {
							$_45+=$total_akhir_o;
							$pdf->Cell($w[8],$h,'Rp. '.separator_harga2($row->sisa_o),'LR',0,'C',$fill);
            } else {
							$pdf->Cell($w[8],$h,'Rp. 0','LR',0,'C',$fill);
						}
						if ($hitung_umur>46 && $hitung_umur<=60) {
							$_60+=$total_akhir_o;
							$pdf->Cell($w[9],$h,'Rp. '.separator_harga2($row->sisa_o),'LR',0,'C',$fill);
            } else {
							$pdf->Cell($w[9],$h,'Rp. 0','LR',0,'C',$fill);
						}
						if ($hitung_umur>61 && $hitung_umur<=90) {
							$_90+=$total_akhir_o;
							$pdf->Cell($w[10],$h,'Rp. '.separator_harga2($row->sisa_o),'LR',0,'C',$fill);
            } else {
							$pdf->Cell($w[10],$h,'Rp. 0','LR',0,'C',$fill);
						}
						if ($hitung_umur>90) {
							$__90+=$total_akhir_o;
							$pdf->Cell($w[11],$h,'Rp. '.separator_harga2($row->sisa_o),'LR',0,'C',$fill);
            } else {
					...
					$c = array(50,30);
					$pdf->SetFont('Times','',9);
					$pdf->Cell($c[0],$h,'Total Umur Piutang 1-15 hari',1,0,'C');
					$pdf->Cell($c[1],$h,'Rp. '.separator_harga2($_15),1,0,'C');
					$pdf->Ln(7);
					$pdf->Cell($c[0],$h,'Total Umur Piutang 16-30 hari',1,0,'C');
					$pdf->Cell($c[1],$h,'Rp. '.separator_harga2($_30),1,0,'C');
					$pdf->Ln(7);
					$pdf->Cell($c[0],$h,'Total Umur Piutang 31-45 hari',1,0,'C');
					$pdf->Cell($c[1],$h,'Rp. '.separator_harga2($_45),1,0,'C');
					$pdf->Ln(7);
					$pdf->Cell($c[0],$h,'Total Umur Piutang 46-60 hari',1,0,'C');
					$pdf->Cell($c[1],$h,'Rp. '.separator_harga2($_60),1,0,'C');
					$pdf->Ln(7);
					$pdf->Cell($c[0],$h,'Total Umur Piutang 61-90 hari',1,0,'C');
					$pdf->Cell($c[1],$h,'Rp. '.separator_harga2($_90),1,0,'C');
					$pdf->Ln(7);
					$pdf->Cell($c[0],$h,'Total Umur Piutang > 90 hari',1,0,'C');
					$pdf->Cell($c[1],$h,'Rp. '.separator_harga2($__90),1,0,'C');
					$pdf->Ln(7);
					$pdf->Cell($c[0],$h,'Total Piutang ',1,0,'C');
				...
				//}
				$pdf->Output('Lap_Umur_Piutang - '.date('d-m-Y').'.pdf','I');
			}else{
				redirect('henkel_adm_lap_umur_piutang');
			}
	...
	public function cetak_excel_piutang(){
		...
		if(!empty($cek) && $level=='henkel_admin'){
				$tgl_awal  = $this->input->get('tgl_awal');
				$tgl_akhir = $this->input->get('tgl_akhir');
        $ambildata = $this->model_lap_piutang->data_laporan_umur_piutang($tgl_awal, $tgl_akhir);
        ...
        if($r>0){
            ...
            $objPHPExcel->getProperties()
                    		...
                    		->setTitle("Export Excel Data Umur Piutang");  //file title

            $objset = $objPHPExcel->setActiveSheetIndex(0); //inisiasi set object
            $objget = $objPHPExcel->getActiveSheet();  //inisiasi get object

            $objget->setTitle('Laporan Umur Piutang');

            //table header
						$cols = array("A","B","C","D","E","F","G","H","I","J","K","L");
						$val = array("NO","Tanggal Transaksi","No. Transaksi","Kode Pelanggan","Jatuh Tempo","Umur Piutang","1-15 Hari","16-30 Hari","31-45 Hari","46-60 Hari","61-90 Hari","> 90 Hari");
            ...
            foreach ($ambildata->result() as $dt){
		          ...
		          $hitung_umur = $interval->format('%a');
								...
								$objset->setCellValue("F".$baris, $umur_piutang_total);
								if ($hitung_umur<=15) {
									$_15+=$total_akhir_o;
									$objset->setCellValue("G".$baris, $dt->sisa_o);
		            } else {
								...
								if ($hitung_umur>31 && $hitung_umur<=45) {
									$_45+=$total_akhir_o;
									$objset->setCellValue("I".$baris, $dt->sisa_o);
		            } else {
									$objset->setCellValue("I".$baris, '0');
								}
								if ($hitung_umur>46 && $hitung_umur<=60) {
									$_60+=$total_akhir_o;
									$objset->setCellValue("J".$baris, $dt->sisa_o);
		            } else {
									$objset->setCellValue("J".$baris, '0');
								}
								if ($hitung_umur>61 && $hitung_umur<=90) {
									$_90+=$total_akhir_o;
									$objset->setCellValue("K".$baris, $dt->sisa_o);
		            } else {
									$objset->setCellValue("K".$baris, '0');
								}
								if ($hitung_umur>90) {
									$__90+=$total_akhir_o;
									$objset->setCellValue("L".$baris, $dt->sisa_o);
		            } else {
                ...
                $baris++;
						...
						$ket_umur = array("Total Umur Piutang 1-15 Hari","Total Umur Piutang 16-30 Hari","Total Umur Piutang 31-45 Hari","Total Umur Piutang 46-60 Hari","Total Umur Piutang 61-90 Hari","Total Umur Piutang > 90 Hari");
            ...
            $objPHPExcel->getActiveSheet()->setTitle('Laporan Umur Piutang');

            $objPHPExcel->setActiveSheetIndex(0);
            $filename = urlencode("Data Laporan Umur Piutang".date("Y-m-d H:i:s").".xls");
            ...
            $objWriter->save('php://output');
					...
					redirect('henkel_adm_lap_umur_piutang', 'refresh');
        }
			}else{
				redirect('henkel','refresh');
			}

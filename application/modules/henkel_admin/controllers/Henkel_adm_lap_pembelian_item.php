...
class Henkel_adm_lap_pembelian_item extends CI_Controller {
	...
	public function index()
	{
		...
		if(!empty($cek)){
			$d['judul']="Laporan Pembelian Per Item";
			$d['class'] = "laporan";
			$d['data_perusahaan'] = '';
			$d['data_status'] = '';
			$d['content']= 'laporan/pembelian/lap_pembelian_item';
	...
	public function cari_data()
	{
		...
		if(!empty($cek)){
			...
			$q = $this->model_lap_pembelian->data_laporan_pembelian_item($tgl_awal, $tgl_akhir);
			$r = $q->num_rows();
			if ($r<1) {
				echo $this->load->view('laporan/view_kosong');
			} else {
				$dt['data'] = $q;
				echo $this->load->view('laporan/pembelian/view_lap_pembelian_item',$dt);
			}
			...
			if($r>0){
			  ...
			  $pdf->AddPage("P","A4");
				//foreach($data->result() as $t){
					...
					$Q[0]=216;
					$Q[1]=279;
					$pdf->SetTitle('Laporan Pembelian Item');
					...
					$pdf->SetX(6);
					$pdf->Cell(190,4,'Laporan Pembelian Per Item',0,1,'C');
					$pdf->Ln(2);
					$pdf->SetFont('Times','',10);
					$pdf->SetX(6);
				...
				//}
				$pdf->Output('Lap_Pembelian_Item - '.date('d-m-Y').'.pdf','I');
			}else{
				redirect('henkel_adm_lap_pembelian_item');
			}
	...
	public function cetak_excel_pembelian(){
		...
		if(!empty($cek) && $level=='henkel_admin'){
				$tgl_awal  = $this->input->get('tgl_awal');
				$tgl_akhir = $this->input->get('tgl_akhir');
        $ambildata = $this->model_lap_pembelian->data_laporan_pembelian_item($tgl_awal, $tgl_akhir);
        ...
        if($r>0){
            ...
            $val = array("NO","Kode Item","Nama Item","Tipe","Jumlah","Harga");
            ...
            foreach ($ambildata->result() as $dt){
							  $harga = $dt->total_item_item_masuk*$dt->harga_tebus_dpp;
								...
								$objset->setCellValue("B".$baris, $dt->kode_item);
								$objset->setCellValue("C".$baris, $dt->nama_item);
								$objset->setCellValue("D".$baris, $dt->tipe);
								$objset->setCellValue("E".$baris, $dt->total_item_item_masuk);
								$objset->setCellValue("F".$baris, $harga);

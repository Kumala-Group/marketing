...
$(document).ready(function(){
        ...
        $("#simpan_spv").click(function(){
            var range_hari_awal = $("#range_hari_awal_spv").val();
            var range_hari_akhir = $("#range_hari_akhir_spv").val();
            var retail_persen = $("#retail_persen").val();
            var toko_persen = $("#toko_persen").val();

            var string = $("#my-form-spv").serialize();


            if(retail_persen.length==0){
                $.gritter.add({
                    title: 'Peringatan..!!',
                    text: 'Retail tidak boleh kosong',
                    class_name: 'gritter-error'
                });
            ...
            if(toko_persen.length==0){
                $.gritter.add({
                    title: 'Peringatan..!!',
                    text: 'Toko tidak boleh kosong',
                    class_name: 'gritter-error'
                });
            ...
            $.ajax({
                type    : 'POST',
                url     : "<?php echo site_url(); ?>henkel_adm_hitungan_komisi/t_simpan_spv",
                data    : string,
                cache   : false,
                start   : $("#simpan_spv").html('Sedang diproses...'),
                success : function(data){
                    $("#simpan_spv").html('<i class="icon-save"></i> Simpan');
                    alert(data);
                    location.reload();
                }
            });
        ...
        $("#simpan_komisi").click(function(){
            ...
            $.ajax({
                type    : 'POST',
                url     : "<?php echo site_url(); ?>henkel_adm_hitungan_komisi/cek_table",
                data    : "id="+id,
                success : function(data){
                  if(data==0){
                    $.gritter.add({
                    ...
                    $.ajax({
                        type    : 'POST',
                        url     : "<?php echo site_url(); ?>henkel_adm_hitungan_komisi/simpan",
                        data    : string,
                        cache   : false,
        ...
        $("#tambah_spv").click(function(){
          $("#id_form_spv").val('<?php echo $id_komisi ?>');
          $("#id_form_spv_komisi").val('<?php echo $id_komisi_spv?>');
          $("#range_hari_awal_spv").val('');
          $("#range_hari_akhir_spv").val('');
          $("#retail_persen").val('');
          $("#toko_persen").val('');
        });
        ...
        $(window).on('beforeunload', function(){
          localStorage.setItem('nama_komisi', $('#nama_komisi').val());
        });
});
  ...
  $.ajax({
      ...
      success : function(data){
        if(data==0){
          ...
          var nama_komisi = localStorage.getItem('nama_komisi');
...
function editData_sales(ID){
  ...
  $.ajax({
    type  : "GET",
    url   : "<?php echo site_url(); ?>henkel_adm_hitungan_komisi/t_cari_sales",
    data  : "cari="+cari,
    dataType: "json",
    success : function(data){
      $('#id_form_sales').val(data.id_komisi);
      $('#id_form_sales_komisi').val(data.id_komisi_sales);
      $('#id_form_sales_t_komisi').val(data.id_t_komisi_sales);
...
function editData_spv(ID){
    var cari  = ID;
    console.log(cari);
  $.ajax({
    type  : "GET",
    url   : "<?php echo site_url(); ?>henkel_adm_hitungan_komisi/t_cari_spv",
    data  : "cari="+cari,
    dataType: "json",
    success : function(data){
      $('#id_form_spv').val(data.id_komisi);
      $('#id_form_spv_komisi').val(data.id_komisi_spv);
      $('#id_form_spv_t_komisi').val(data.id_t_komisi_spv);
      $('#range_hari_awal_spv').val(data.range_hari_awal);
      $('#range_hari_akhir_spv').val(data.range_hari_akhir);
      $('#retail_persen').val(data.retail_persen);
      $('#toko_persen').val(data.toko_persen);
    }
  });
}
...
function editData_admin(ID){
  ...
  $.ajax({
    type  : "GET",
    url   : "<?php echo site_url(); ?>henkel_adm_hitungan_komisi/t_cari_admin",
    data  : "cari="+cari,
    dataType: "json",
    success : function(data){
      $('#id_form_admin').val(data.id_komisi);
      $('#id_form_admin_komisi').val(data.id_komisi_admin);
      $('#id_form_admin_t_komisi').val(data.id_t_komisi_admin);
...
function hapusData_sales(id){
  ...
  $.ajax({
    		type	: "POST",
    		url		: "<?php echo site_url(); ?>henkel_adm_hitungan_komisi/t_hapus_sales/"+id,
    		data	: "id_h="+id,
    		dataType: "json",
    		success	: function(){
          location.reload();
 ...
 function hapusData_spv(id){
   var r = confirm("Anda yakin ingin menghapus data ini?");
   if (r == true) {
   $.ajax({
     		type	: "POST",
     		url		: "<?php echo site_url(); ?>henkel_adm_hitungan_komisi/t_hapus_spv/"+id,
     		data	: "id_h="+id,
     		dataType: "json",
     		success	: function(){
           location.reload();
 ...
 function hapusData_admin(id){
   ...
   $.ajax({
     		type	: "POST",
     		url		: "<?php echo site_url(); ?>henkel_adm_hitungan_komisi/t_hapus_admin/"+id,
     		data	: "id_h="+id,
     		dataType: "json",
     		success	: function(){
           location.reload();
...
<div class="table-header">
    ...
    <input type="hidden" value="<?php echo $id_komisi_sales;?>" name="id_sales" id="id_sales">
    <input type="hidden" value="<?php echo $id_komisi_spv;?>" name="id_spv" id="id_spv">
    <input type="hidden" value="<?php echo $id_komisi_admin;?>" name="id_admin" id="id_admin">
</div>
<br />
<?php
        ...
        <tr>
            ...
            <td class="td-actions"><center>
                <div class="hidden-phone visible-desktop action-buttons">
                    <a class="green" href="#modal-table-sales" onclick="javascript:editData_sales('<?php echo $dt_sales->id_t_komisi_sales;?>')" data-toggle="modal">
                        <i class="icon-pencil bigger-130"></i>
                    </a>

                    <a class="red" href="#" onclick="javascript:hapusData_sales('<?php echo $dt_sales->id_t_komisi_sales;?>')">
...
<!--<div class="table-header">
 Tabel Item Komisi (Supervisor/Operational Manager)
 <div class="widget-toolbar no-border pull-right">
   <a href="#modal-table-spv" data-toggle="modal" name="tambah_spv" id="tambah_spv" class="btn btn-small btn-success">
       <i class="icon-plus"></i>
       Tambah Item
   </a>
 </div>
...
<!--<table class="table table-striped table-bordered table-hover">
    ...
    <tbody>
        <?php
        $i=1;
        foreach($data_spv->result() as $dt_spv){ ?>
        <tr>
            <td class="center span1"><?php echo $i++?></td>
            <td class="center"><?php echo $dt_spv->range_hari_awal.' - '.$dt_spv->range_hari_akhir;?></td>
            <td class="center"><?php echo $dt_spv->retail;?></td>
            <td class="center"><?php echo $dt_spv->toko;?></td>
            <td class="td-actions"><center>
                <div class="hidden-phone visible-desktop action-buttons">
                    <a class="green" href="#modal-table-spv" onclick="javascript:editData_spv('<?php echo $dt_spv->id_t_komisi_spv;?>')" data-toggle="modal">
                        <i class="icon-pencil bigger-130"></i>
                    </a>

                    <a class="red" href="#" onclick="javascript:hapusData_spv('<?php echo $dt_spv->id_t_komisi_spv;?>')">
                        <i class="icon-trash bigger-130"></i>
                    </a>
                </div>
...
<table class="table table-striped table-bordered table-hover">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td class="td-actions"><center>
                <div class="hidden-phone visible-desktop action-buttons">
                    <a class="green" href="#modal-table-admin" onclick="javascript:editData_admin('<?php echo $dt_admin->id_t_komisi_admin;?>')" data-toggle="modal">
                        <i class="icon-pencil bigger-130"></i>
                    </a>

                    <a class="red" href="#" onclick="javascript:hapusData_admin('<?php echo $dt_admin->id_t_komisi_admin;?>')">
                        <i class="icon-trash bigger-130"></i>
                    </a>
                </div>
    ...
    <div class="modal-body no-padding">
        <div class="row-fluid">
            <form class="form-horizontal" name="my-form-sales" id="my-form-sales">
              ...
              <input type="hidden" name="id_form_sales_t_komisi" id="id_form_sales_t_komisi">
            <br>
                <div class="control-group">
...
<div id="modal-table-spv" class="modal hide fade" tabindex="-1" data-backdrop="static" data-keyboard="false">
    <div class="modal-header no-padding">
        <div class="table-header">
            <button type="button" class="close" data-dismiss="modal">&times;</button>
            Tambah Item
    ...
    <div class="modal-body no-padding">
        <div class="row-fluid">
            <form class="form-horizontal" name="my-form-spv" id="my-form-spv">
            <input type="hidden" name="id_form_spv" id="id_form_spv">
            <input type="hidden" name="id_form_spv_t_komisi" id="id_form_spv_t_komisi">
            <br>
            <div class="control-group">
                <label class="control-label" for="form-field-1">TOP</label>

                <div class="controls">
                    <input type="text" class="span4" name="range_hari_awal_spv" id="range_hari_awal_spv" onkeydown="return justAngka(event)" placeholder="Awal" /> -
                    <input type="text" class="span4" name="range_hari_akhir_spv" id="range_hari_akhir_spv" onkeydown="return justAngka(event)" placeholder="Akhir" /> <span style="font-weight:bold"> Hari</span>
                </div>
            </div>
            <div class="control-group">
                <label class="control-label" for="form-field-1">Retail (%)</label>

                <div class="controls">
                    <input type="text" name="retail_persen" id="retail_persen" onkeydown="return justAngka(event)" placeholder="Retail (%)"/> <span style="font-weight:bold"> %</span>
                </div>
            </div>
            <div class="control-group">
                <label class="control-label" for="form-field-1">Toko (%)</label>

                <div class="controls">
                    <input type="text" name="toko_persen" id="toko_persen" onkeydown="return justAngka(event)" placeholder="Toko (%)"/> <span style="font-weight:bold"> %</span>
                </div>
            </div>
          </div>
                <div class="modal-footer">
                    ...
                    <button type="button" name="simpan_spv" id="simpan_spv" class="btn btn-small btn-success pull-left">
                        <i class="icon-save"></i>
                        Simpan
                    </button>
                    </div>

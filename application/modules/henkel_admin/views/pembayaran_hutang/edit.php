...
$(document).ready(function(){
        ...
        $("#simpan").click(function(){
            ...
            $.ajax({
                type    : 'POST',
                url     : "<?php echo site_url(); ?>henkel_adm_hutang/simpan",
                data    : string,
                cache   : false,
                success : function(data){
                    alert(data);
        ...
        $("#simpan_pembayaran_hutang").click(function(){
            ...
            $.ajax({
                ...
                success : function(data){
                    $.ajax({
                        type    : 'POST',
                        url     : "<?php echo site_url(); ?>henkel_adm_pembayaran_hutang/simpan",
                        data    : string,
                        cache   : false,
                        success : function(data){
                          var id= $("#id").val();
                          alert(data);
                          location.replace("<?php echo site_url(); ?>henkel_adm_pembayaran_hutang")
                        }
                    });
                }
            });
        ...
        $("#cetak").click(function(){
          $("#cetak_hutang").val(cetak_hutang++);
          cetak();
        });

        $("#cetak_permintaan_pembayaran").click(function(){
          cetak_permintaan_pembayaran();
        });
    ...
    function editData(ID){
    	...
    	$.ajax({
    		type	: "GET",
    		url		: "<?php echo site_url(); ?>henkel_adm_hutang/cari",
    		data	: "cari="+cari,
    		dataType: "json",
    		success	: function(data){
          var id = $("#id").val();
    			$('#id_pembayaran_hutang').val(id);
    			$('#id_hutang').val(data.id_hutang);
          ...
          $('#total').val(f_diskon(total,diskon));
          $('#kembali').val(f_bayar(kredit,bayar));
    		}
    	});
...
function cetak() {
    ...
    var cetak_hutang= $("#cetak_hutang").val();
    window.open("<?php echo site_url(); ?>henkel_adm_pembayaran_hutang/cetak?id="+id+"&no_transaksi="+no_transaksi+"&tanggal="+tgl+"&kode_supplier="+kode_supplier+"&alamat="+alamat+"&nama_supplier="+nama_supplier+"&total_bayar="+total_bayar+"&total_sisa="+total_sisa+"&sisa_kredit="+sisa_kredit+"&keterangan="+keterangan+"&cetak_hutang="+cetak_hutang)
}

function cetak_permintaan_pembayaran() {
    var id= $("#id").val();
    var no_transaksi= $("#no_transaksi").val();
    var tgl= $("#tanggal").val();
    var kode_supplier= $("#kode_supplier").val();
    ...
    var keterangan= $("#keterangan").val();
    var cetak_hutang= $("#cetak_hutang").val();
    window.open("<?php echo site_url(); ?>henkel_adm_pembayaran_hutang/cetak_permintaan_pembayaran?id="+id+"&no_transaksi="+no_transaksi+"&tanggal="+tgl+"&kode_supplier="+kode_supplier+"&alamat="+alamat+"&nama_supplier="+nama_supplier+"&total_bayar="+total_bayar+"&total_sisa="+total_sisa+"&sisa_kredit="+sisa_kredit+"&keterangan="+keterangan+"&cetak_hutang="+cetak_hutang)
}
...
<form class="form-horizontal" name="form_penjualan" id="form_penjualan" action="<?php echo base_url();?>henkel_adm_pembayaran_hutang/cetak" method="post">
<div class="row-fluid">
   ...
   <div class="row-fluid">
        <div class="span12">
                <div class="control-group">
                    ...
                    <div class="controls">
                        <input type="text" class="autocomplete" value="<?php echo $kode_supplier;?>" name="kode_supplier" id="kode_supplier" placeholder="Kode Supplier" readonly/>
                    </div>
               </div>
                <div class="control-group">
                    <label class="control-label" for="form-field-1">Supplier</label>
                    <div class="controls">
                        <input type="text" class="autocomplete" value="<?php echo $nama_supplier;?>" name="nama_supplier" id="nama_supplier" placeholder="Nama Supplier" readonly/>
                    </div>
                </div>
                <div class="control-group">
          ...
          ?>
            ...
            <!--<td class="td-actions"><center>
                <div class="text-phone visible-desktop action-buttons">
                    <a class="green" href="#modal-table" onclick="javascript:editData('<?php echo $dt->id_hutang;?>')" data-toggle="modal">
...
<div class="row-fluid">
  <div class="span6">
    <div class="control-group">
        ...
        <div class="controls">
            <input type="text" class="span12"  value="<?php echo $keterangan; ?>" name="keterangan" id="keterangan" placeholder="Keterangan" readonly="readonly"/>
        </div>
   </div>
  </div>
  <div class="span6 ">
    <div class="control-group">
        ...
        <div class="controls total_harga">
          ...
          <div class="control-group">
              ...
              <div class="controls total_harga">
                  <input type="text" class="span12" style="text-align: right;" value="<?php echo separator_harga2($sisa_kredit); ?>" min='0' name="total_kredit" id="total_kredit" placeholder="0" readonly="readonly"/>
              </div>
         </div>
        <input type="hidden" value="0" min="0" name="cetak_hutang" id="cetak_hutang" readonly="readonly" />
   </div>
 </div>
 <br>
 <div class="row-fluid">
...
<div class="row-fluid">
     <div class="span12" align="center">
       ...
       <!--<button type="button" name="cetak_permintaan_pembayaran" id="cetak_permintaan_pembayaran" class="btn btn-small btn-info">
       <i class="icon-print"></i> Cetak Permintaan Pembayaran
     </button>-->

...
$(document).ready(function(){
        ...
        $("#cari_kode_item").click(function(){
            $.ajax({
              url   : "<?php echo site_url();?>henkel_adm_datatable/item_master",
              dataType: "json",
              success : function(data){
                        table = $('#show_item').DataTable({
                        "bProcessing": true,
                        "bDestroy": true,
                        "sAjaxSource": '<?php echo site_url();?>henkel_adm_datatable/item_master',
                        "bSort": false,
                         "bAutoWidth": true,
          ...
          $('#show_item tbody').on( 'click', 'tr', function () {
              ...
              var tipe=$(this).find('td').eq(3).text();
        ...
        $("#simpan").click(function(){
            var tanggal_item_masuk_non = $("#tanggal_item_masuk_non").val();
            ...
            if(tanggal_item_masuk_non.length==0){
                $.gritter.add({
                    title: 'Peringatan..!!',
           ...
           if(keterangan.length==0){
            ...
            $.ajax({
                type    : 'POST',
                url     : "<?php echo site_url(); ?>henkel_adm_item_masuk_non/simpan",
                data    : string,
                cache   : false,
                success : function(data){
                    alert(data);
        ...
        $("#tambah").click(function(){
          ...
          $("#id_item_masuk_non").val('');
          $("#tanggal_item_masuk_non").val('');
          $('#kode_item_masuk_non').val('<?php echo $kode_item_masuk_non?>');
          $("#kode_gudang").val('');
          $("#kode_item").val('');
          $("#nama_item").val('');
          $("#tipe").val('');
...
function editData(ID){
  ...
  $.ajax({
    type  : "GET",
    url   : "<?php echo site_url(); ?>henkel_adm_item_masuk_non/cari",
    data  : "cari="+cari,
    dataType: "json",
    success : function(data){
      $('#id_item_masuk_non').val(data.id_item_masuk_non);
      $('#kode_item_masuk_non').val(data.kode_item_masuk_non);
      $('#tanggal_item_masuk_non').val(data.tanggal_item_masuk_non);
...
function hapusData(id){
  ...
  $.ajax({
        type  : "POST",
        url   : "<?php echo site_url(); ?>henkel_adm_item_masuk_non/hapus/"+id,
        data  : "id_h="+id,
        dataType: "json",
        success : function(){
          location.reload();
...
function search_nm_item(){
  ...
  $.ajax({
    type  : "POST",
    url   : "<?php echo site_url(); ?>henkel_adm_item_masuk_non/search_nm_item",
...
function beritaAcara(ID){
  ...
  $.ajax({
    type  : "GET",
    url   : "<?php echo site_url(); ?>henkel_adm_item_masuk_non/cari_berita_acara",
    data  : "cari="+cari,
    dataType: "json",
    success : function(data){
       window.open("<?php echo site_url(); ?>henkel_adm_item_masuk_non/berita_acara?id_item_keluar="+btoa(data.id_item_keluar)+"&kode_item_keluar="+btoa(data.kode_item_keluar)+"&kode_gudang="+btoa(data.kode_gudang)+"&kode_item="+btoa(data.kode_item)+"&nama_item="+data.nama_item+"&jumlah="+data.jumlah+"&satuan="+data.satuan+"&tanggal_item_keluar="+btoa(data.tanggal_item_keluar)+"&tanggal_input="+data.tanggal_input+"&keterangan="+data.keterangan);
    }
  });
}
</script>
...
<table class="table fpTable lcnp table-striped table-bordered table-hover">
    <thead>
        <tr>
            ...
            <th class="center">Tanggal Item Masuk</th>
    ...
    <tbody>
        ...
        <tr>
          ...
          ?>
            ...
            <td class="center"><?php echo tgl_sql($dt->tanggal_item_masuk_non);?></td>
            ...
            <td class="td-actions"><center>
                <div class="hidden-phone visible-desktop action-buttons">
                    <!--<a class="green" href="#modal-table" onclick="javascript:editData('<?php echo $dt->id_item_masuk_non;?>')" data-toggle="modal">
                        <i class="icon-pencil bigger-130"></i>
                    </a>-->

                    <a class="red" href="#" onclick="javascript:hapusData('<?php echo $dt->id_item_masuk_non;?>')">
                        <i class="icon-trash bigger-130"></i>
                    </a>

                    <!--<a href="#" onclick="javascript:beritaAcara('<?php echo $dt->id_item_masuk_non_non;?>')">
...
<div id="modal-table" class="modal hide fade" tabindex="-1" data-backdrop="static" data-keyboard="false">
    ...
    <div class="modal-body no-padding">
        <div class="row-fluid">
            ...
            <input type="hidden" name="id_item_masuk_non" id="id_item_masuk_non">
            <input type="hidden" name="kode_item_masuk_non" id="kode_item_masuk_non">
            <br>
                <div class="control-group">
                    <label class="control-label" for="form-field-1">Tanggal Item Masuk</label>
                    <div class="controls">
                      <div class="input-append">
                        <input type="text" name="tanggal_item_masuk_non" id="tanggal_item_masuk_non" class="date-picker"  data-date-format="dd-mm-yyyy" placeholder="Tanggal Item Masuk"/>
                      </div>
                    </div>
                </div>
                <div class="control-group">

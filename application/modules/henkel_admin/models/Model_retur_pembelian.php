...
class Model_retur_pembelian extends CI_Model {
 	...
 	public function all(){
		$q = $this->db_kpp->query('SELECT rp.*,s.nama_supplier
                               FROM retur_pembelian rp
                               JOIN supplier s
                               ON rp.kode_supplier=s.kode_supplier
                               ORDER BY id_retur_pembelian ASC');
		return $q;
	}
  ...
  public function ada_transaksi($id){
    $q 	 = $this->db_kpp->query("SELECT no_invoice FROM item_masuk WHERE no_invoice='$id'");
		$row = $q->num_rows();
		return $row > 0;
	}

  function getIdItemMasuk($id)
	{
		$this->db_kpp->select('id_item_masuk');
		$this->db_kpp->where('no_invoice', $id);
		$q = $this->db_kpp->get('item_masuk');
		//if id is unique we want just one row to be returned
		$data = array_shift($q->result_array());
		$result = $data['id_item_masuk'];
		return  $result;
	}

  function get_data_pembelian($id)
	{
    $this->db_kpp->select('total_akhir,bayar,status,kredit_o');
		$this->_item->where('no_po', $id);
		$q = $this->db_kpp->get('pesanan_pembelian');
    $rows = $q->num_rows();
  ...
  public function last_kode(){
    $q = $this->db_kpp->query("SELECT MAX(right(no_retur,3)) as kode FROM retur_pembelian ");
    $row = $q->num_rows();
  ...
  public function getByIdRPembelian($id_retur_pembelian){
		$id['id_retur_pembelian'] = $id_retur_pembelian;
		$q = $this->db_kpp->get_where("retur_pembelian",$id);
		$rows = $q->num_rows();
  ...
  public function delpes($id){
	...
	public function cari_max_retur_pembelian(){
		$q = $this->db_kpp->query("SELECT MAX(id_retur_pembelian) as no FROM retur_pembelian");
 	...
 	public function insert($dt){
 		$this->db_kpp->insert("retur_pembelian",$dt);
 	}

 	public function update($id, $dt){
 		$this->db_kpp->update("retur_pembelian",$dt,$id);
 	}

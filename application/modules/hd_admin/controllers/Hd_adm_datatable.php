...
class Hd_adm_datatable extends CI_Controller {
	...
	public function no_penjualan()
    {
		...
		public function no_po()
	    {
			...
			public function nik_karyawan_lama()
		    {
						$idper=$this->session->userdata('id_perusahaan');
		    ...
		    {
		        $list = $this->db->query("SELECT a.nik,a.nama_karyawan,a.id_perusahaan,b.lokasi,c.nama_brand FROM karyawan a, 
				perusahaan b, brand c WHERE status_aktif='aktif'AND a.id_perusahaan=b.id_perusahaan AND a.id_brand=c.id_brand");
		        $row = array();
		        $no = 1;
		        foreach ($list->result() as $dt) {
		            $row[] = array(
									...
									'nik'=>$dt->nik,
									'nama_karyawan'=>'<center>'.$dt->nama_karyawan.'</center>',
									'id_perusahaan'=>'<center>'.$dt->id_perusahaan.'</center>',
				...
				public function nik_karyawan_semua()
			    {
			        $list = $this->db->query("SELECT nik,nama_karyawan
																						FROM karyawan");
			        $row = array();

...
class Hd_adm_update_ban extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if (!empty($cek) && $level == 'karyawan') {
            $brand = $this->db->query("SELECT nama_brand FROM kmg.brand WHERE id_brand ='8'")->row()->nama_brand;
            $d['judul'] = " DMS UPDATE VERSI " . $brand;
            $d['class'] = "dms update";
            $d['data'] = $this->model_update_versi->all($brand);
            $d['content'] = 'dms_update/ban/view';
